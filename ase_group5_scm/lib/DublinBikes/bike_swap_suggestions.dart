...
class BikeSwapSuggestions extends StatefulWidget {
  ...
  const BikeSwapSuggestions({Key? key, required this.snapshot})
      : super(key: key);
...
class _BikeSwapSuggestionsState extends State<BikeSwapSuggestions> {
  ...
  Widget build(BuildContext context) {
    var bike_swap_suggestions =
        widget.snapshot.docs[0].get("swap_suggestions")[0];
    return Container(
        ...
        child: Column(
          ...
          children: [
            SizedBox(
              ...
              child: generateDataTable2(
                  bike_swap_suggestions["occupied_stations"]),
              color: Color(0xFF9CBFC9),
            ...
            SizedBox(
              child: Container(
                ...
                width: double.infinity,
                child:
                    generateDataTable(bike_swap_suggestions["free_stations"]),
                color: Color(0xFF9CBFC9),
                alignment: Alignment.center,
              ),
            )
...
DataTable2 generateDataTable(bike_swap_suggestions) {
  return DataTable2(
    ...
    rows: List.generate(
        5, (index) => generateSuggestionRow(bike_swap_suggestions[index])),
  );
}
