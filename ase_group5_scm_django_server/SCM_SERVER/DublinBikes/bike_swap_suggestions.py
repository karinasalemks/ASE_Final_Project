...
def proprocessBikeStationData():
    ...
    for station_id in stations_list:
        ...
        distance_vector = sorted(distance_vector.items(), key=lambda item: item[1])
...
def generate_swap_suggestions(bikeStationData,distance_matrix):
    ...
    top_5_occupied_stations = top_5_occupied_stations[-5:0]
    top_5_free_stations = top_5_free_stations[:5]

    #Filter table and find the nearest station for each station
    for occupied_station in top_5_occupied_stations:
        station_id = occupied_station[0]
